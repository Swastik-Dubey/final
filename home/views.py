    ...
    try :
        ...
        for i in db1.each() :
            if(i.val()['courier']==userid) :
                ...
                return render(request , "courierorder.html",{"courierdetails":i.val(),"user_id":userid})
            else:
...
def confirmedcourier(request):    
    
    bill_id=request.POST.get("bill_id" , None)
    userid2=request.POST.get("hiddenuserid5")
...
def postconfirmcourier(request) :
    ...
    for pcdispatch in dbdispatch.each() :
        if pcdispatch.val()['Bill Id'] == bill_id : 
            fromcity = database.child("Data").child("ConfirmedOrders").child("OrderDetails").child("dummydispatch").child(pcdispatch.key()).child("fromcity").get().val()
            companyname12=database.child("Data").child("ConfirmedOrders").child("OrderDetails").child("dummydispatch").child(pcdispatch.key()).child("company_name").get().val()
            datee = database.child("Data").child("ConfirmedOrders").child("OrderDetails").child("dummydispatch").child(pcdispatch.key()).child("date").get().val()
            destination = database.child("Data").child("ConfirmedOrders").child("OrderDetails").child("dummydispatch").child(pcdispatch.key()).child("destination").get().val()
    ...
    for deletedispatch in dbdispatch.each():
        if deletedispatch.val()['Bill Id'] == bill_id :
            database.child("Data").child("ConfirmedOrders").child("OrderDetails").child("dummycourier").child(deletedispatch.key()).remove()
        firebase=FirebaseApplication("https://neemeesh-trial-default-rtdb.firebaseio.com/", None)
    fromcity=list(firebase.get("/Data/ConfirmedOrders/OrderDetails/dummydispatch",None).values())
    fromcitylist=[]
...
def postbookingorder(request):
    if request.method == 'POST':
        if "Next" in request.POST:
            ...
            database.child("Data").child(user_id).child("TempBooking").push(data)
            return render (request , "bookingorder.html" , { "fromcity" : fromcity ,
                                                            "company_name" : company_name,
                                                            "datee" : datee , 
                                                            "user_id" :user_id,
                                                            "msg":mssg})
            ...
            bill_id = uuid.uuid4()
            company_name = request.POST.get("company_name")
            db9 = database.child("Data").child("Company").get()
            for i in db9.each() :
                if i.val()["Company Name"] == company_name : 
                    cost=database.child("Data").child("Company").child(i.key()).child("Cost").get().val()
            ...
            database.child("Data").child(user_id).child("TempBooking").push(data)
            list1=[]
            db8 = database.child("Data").child(user_id).child("TempBooking").get()
            for i in db8.each():
                list1=list1+[[i.val()["company_name"],i.val()["date"],i.val()["description"],i.val()["destination"],i.val()["docket_no"],i.val()["fromcity"],i.val()["noofpckg"],i.val()["partyname"],i.val()["totalcost"]]]
                database.child("Data").child("BookingOrder").child("Orders").push(i.val())
                database.child("Data").child("DummyBookingOrder").child(user_id).push(i.val())
            docno=[]
            invcno=[]
            for j in list1 :
                docno.append(j[4])
            ...
            msg = "Your Order for "+list1[0][0]+" dated on "+str(list1[0][1])+"\n"+"Order Details : \n"
            for x in range(len(docno)):
                msg = msg + "Order Number"+str((x+1))+":\n"+"    Docket Number : "+str(docno[x])+"\n     "+"Invoice Number : "+str(invcno[x])+"\n"
            ...
            return render (request , "bookingorder1.html" , {"user_id" :user_id ,"msg":mssg})
   

def postdonebookingorder(request):
    company_name = request.POST.get("company_name")
    db9 = database.child("Data").child("Product").get()
    for i in db9.each() :
        if i.val()["Company Name"] == company_name : 
